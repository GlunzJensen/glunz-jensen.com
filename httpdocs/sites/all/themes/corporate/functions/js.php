...
Nukes the js
*/
function corporate_js_alter(&$js) {
  //js from core modules
  if (theme_get_setting('corporate_js_nuke_module')){
  ...
  //js from contrib
  if (theme_get_setting('corporate_js_nuke_module_contrib')){
    foreach ($js as $file => $value) {
      if (strpos($file, '/modules/') !== FALSE) {
        unset($js[$file]);
      }
  ...
  if (theme_get_setting('corporate_js_nuke_misc')){
    foreach ($js as $file => $value) {
  ...
  $js_kill_list = explode("\n", theme_get_setting('corporate_js_freeform'));

  //grap the css and run through em
  if(theme_get_setting('corporate_js_freeform')){
    foreach ($js as $file => $value) {
  ...
  if(theme_get_setting('corporate_js_jquerycdn')){
  ...
  if(theme_get_setting('corporate_js_onefile')){
